...
from collections import Counter # カウント用にインポートが必要ですが、collectionsは既にimport済みのはずです

def run_recognition_test(cap, shapes_db):
    """
    テストモード：認識成功したものを表示
    ★追加機能：同じIDが複数検出されたら「赤枠」で警告
    """
    ...
    print("Press [SPACE] to confirm and proceed.")
    ...
    while True:
        ...
        # 案内テキスト
        #cv2.putText(display_img, "** TEST MODE **", (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 255, 255), 2)
        #cv2.putText(display_img, status_text, (20, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (200, 200, 200), 2)
...
def main():
    ...
    while True:
        ...
        # -----------------------------------------------------------
        # ★修正：完了後に「手が画面外へ」→「ロック表示消す」→「手が戻る」で次へ
        # -----------------------------------------------------------
        if is_tracking_locked:
            if hand_pos is None:
                # 手が見えない場合、タイマーを進める（一瞬の見切れ防止）
                hand_missing_timer += 1
                if hand_missing_timer > HAND_MISSING_WAIT:
                    # 十分な時間消えていたら「外出中」とマーク
                    ready_for_next_on_return = True
                    
                    # ★修正箇所：手が消えたら右画面のロック表示（輪郭）も消す
                    locked_pieces_data = [] 
            else:
                # 手が見えている場合
                hand_missing_timer = 0
                
                # もし「外出中（ready_for_next_on_return）」だったなら
                # 「今戻ってきた」ということなので、ターゲットを切り替える
                if ready_for_next_on_return:
        ...
        if not is_tracking_locked:
            ...
            for cnt in cnts:
                ...
                # ★修正: 以下のフィルタ処理をコメントアウトして、テストモードと同じ基準にする
            ...
            for p in locked_pieces_data:
                ...
                #cv2.putText(frame_display, f"LOCKED", (p['center'][0], p['center'][1]), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
            
            # 状態表示（デバッグ）
            #if ready_for_next_on_return:
            #     cv2.putText(frame_display, "WAITING FOR HAND ENTRY...", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
            #else:
            #     cv2.putText(frame_display, "HAND IS IN FRAME", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (200, 200, 200), 2)
